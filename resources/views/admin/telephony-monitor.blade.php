...
@push('scripts')
<script>
window.telephonyMonitor = function () {
    return {
        status: @js($status),
        events: [],
        severityFilter: 'all',
        ...
        init() {
            ...
            if (!te || !te.initEcho || !te.isBroadcastEnabled()) return;
            te.initEcho();
            this._unsubscribe = te.subscribeSupervisorChannel(
                () => {},
                () => {},
                (payload) => {
                    this.events.unshift({
                        ...
                        message: payload.message || 'Telephony event received',
