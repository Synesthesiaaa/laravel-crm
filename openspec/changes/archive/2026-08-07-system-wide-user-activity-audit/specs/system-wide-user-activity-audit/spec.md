...
The system SHALL expose request activities through the existing Super Admin Activity Log history endpoint and private realtime channel. The normalized entry SHALL show the request method as its action and include request path, route, status, and actor details. The actor filter SHALL include every current user, not only users with an administrative role.

#### Scenario: Super Admin sees requests from different roles

- **WHEN** a Super Admin opens Activity Log after an Agent, Team Leader, or Admin has made requests
- **THEN** the stream includes those request entries with the correct actor labels
...
- **THEN** only matching request activities are returned within the server-side limit
...
- **THEN** the new normalized request entry is appended to the terminal through the existing realtime/polling mechanism
