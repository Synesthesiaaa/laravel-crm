...
The guest login page SHALL present a minimal authentication composition that uses the CRM's shared surface, text, border, and Signal Magenta tokens, while keeping the sign-in form as the primary visual task.

#### Scenario: Dark theme entry

- **WHEN** a guest opens the login route with the dark theme active
- **THEN** the page renders a charcoal atmosphere and auth sheet with readable primary and muted text, and Signal Magenta is reserved for the primary action, focus, and meaningful emphasis

#### Scenario: Light theme entry

- **WHEN** a guest switches the login page to the light theme
- **THEN** the page rebinds its surfaces, borders, text, and elevation to the existing light-theme tokens without introducing a separate hard-coded palette
...
- **THEN** the brand remains visible in the entry composition, preserves its accessible alternative text, and long names wrap without creating horizontal overflow
...
- **THEN** the page exposes a named main landmark, a connected heading, visible labels for username and password, an optional campaign selector when campaigns exist, and a full-width sign-in action in logical tab order

#### Scenario: Campaign selection is available

- **WHEN** the application provides one or more campaigns
- **THEN** the campaign selector displays the current/old selection using the existing campaign values and remains a native keyboard- and touch-operable control
...
- **THEN** the message is rendered in a readable semantic feedback region without obscuring the fields or moving the submit action off-screen at supported widths
...
- **THEN** the form fits the viewport with safe gutters, the brand/context area stays compact, controls remain at least 44 CSS pixels tall, and no page-level horizontal scrolling is introduced
...
- **THEN** the login page removes entry and hover motion while keeping content visible, focus behavior intact, and state changes understandable
