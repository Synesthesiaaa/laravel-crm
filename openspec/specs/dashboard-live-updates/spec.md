...
channel and MUST refresh its current server-rendered data when a matching update
event arrives. The refresh MUST use the existing soft-navigation boundary, keep
the current URL filters, and preserve the persistent telephony shell.

#### Scenario: A matching campaign event arrives
...
- **WHEN** multiple matching events arrive within the debounce window
